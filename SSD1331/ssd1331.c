...
void ssd1331_draw_3232LowVolt(uint8_t chXpos, uint8_t chYpos, uint16_t hwColor)
{
	uint8_t i, j, k;
	uint8_t chTemp = 0, chYpos0 = chYpos;
...
void ssd1331_draw_3232Warning(uint8_t chXpos, uint8_t chYpos, uint16_t hwColor)
{
	uint8_t i, j, k;
	uint8_t chTemp = 0, chYpos0 = chYpos;
...
/**
  * @brief  清除坐标区域显示
  * @param  startXpos：X起始坐标;startYpos：Y起始坐标;endXpos:X终点坐标;endYpos:Y终点坐标;
  * @retval  false：坐标错误
  */
